...
class ChatBot extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      defaultUserSettings: {},
      typeRecognition: props.typeRecognition,
      recognitionVoice:false
    };
  ...
  componentDidMount() {
    ...
    for (let i = 0, len = steps.length; i < len; i += 1) {
      ...
      } else if (step.message || step.asMessage || step.messageia) {
    ...
    const { recognitionLang, typeRecognition, urlRecognition, timeRecognition } = this.props;
  ...
  componentDidUpdate(prevProps, prevState) {
    if ((prevState.inputValue !== this.state.inputValue) && this.state.recognitionVoice) {
      this.submitUserMessage();
      // Aquí puedes realizar acciones basadas en el cambio del inputValue
    }
  }
  observeContent() {
    this.observer = new MutationObserver(this.onNodeInserted);
  ...
  handleSubmitButton = async () => {
    console.info("Ingreso submit")
    const { speaking, recognitionEnable, typeRecognition } = this.state;
    if (typeRecognition === 0) {
    ...
    if (typeRecognition === 1 || typeRecognition === 2) {
      if ((this.isInputValueEmpty() || speaking) && recognitionEnable) {
        await this.voicerecorder.startRecording()
        if (!speaking) {
          this.setState({ speaking: true, recognitionVoice:true });
        }
        return
      }
  ...
  submitUserMessage = () => {
    console.info("Ingresa submit mensaje")
          ...
          <Footer className="rsc-footer" style={footerStyle}>
            {!currentStep.hideInput && (
              <Input
                ...
                //value={(typeRecognition===0&&!speaking)?inputValue:''}
                value={inputValue}
                floating={floating}
                invalid={inputInvalid}
                disabled={disabled}
...
ChatBot.propTypes = {
  ...
  typeRecognition: PropTypes.number,
  urlRecognition: PropTypes.string,
  timeRecognition: PropTypes.number,
  urlChatIa: PropTypes.string
};
...
ChatBot.defaultProps = {
  ...
  width: '350px',
  typeRecognition: 0,
  urlRecognition: 'http://localhost:5000/transcribe',
